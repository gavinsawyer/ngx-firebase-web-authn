...
export const verifyUserWithPasskey: (auth: Auth, functions: Functions) => Promise<UserCredential> = (auth: Auth, functions: Functions): Promise<UserCredential> => httpsCallableFromURL<FunctionRequest, FunctionResponse>(functions, "/ngxFirebaseWebAuthn")({
  operation: "create reauthentication challenge",
}).then<UserCredential>(async (httpsCallableResult: HttpsCallableResult<FunctionResponse>): Promise<UserCredential> => ((functionResponse: FunctionResponse) => "code" in functionResponse && functionResponse.code ? ((): never => {
...
})).catch<never>(async (): Promise<never> => httpsCallableFromURL<FunctionRequest, FunctionResponse>(functions, "/ngxFirebaseWebAuthn")({
